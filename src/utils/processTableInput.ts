import { getTerminalWidth } from '../tools/printTable';
...
const getDesiredColumnWidths = (cells: Cells, numCols: number) => {
  const transposed = zip(...[...cells.header, ...cells.body]);

  const currColWidths = transposed.map((col) => Math.max(...col.map((cell) => out.utils.getLinesWidth(cell))));
...
export const processInput = (cells: Cells) => {

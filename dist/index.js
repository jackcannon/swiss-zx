...
(($$2) => {
  ...
  $$2.rm = (item) => import_zx.$`rm -rf ${item}`;
  $$2.mkdir = (item) => import_zx.$`mkdir -p ${item}`;
  $$2.cp = (a, b) => import_zx.$`cp -r ${a} ${b}`;
  $$2.mv = (a, b) => import_zx.$`mv ${a} ${b}`;
  $$2.touch = (item) => import_zx.$`touch ${item}`;
  $$2.cat = (item) => import_zx.$`cat ${item}`;
  $$2.grep = async (pattern, file) => utils.intoLines(await import_zx.$`grep ${pattern} ${file}`);
  ...
  $$2.find = async (dir = ".", options = {}) => {
    ...
    return utils.intoLines(result).map(import_swiss_node.PathTools.removeDoubleSlashes).filter(import_swiss_ak.fn.isNotEqual(".")).filter((str) => !str.includes(".Trash")).map(options.removeTrailingSlashes ? import_swiss_node.PathTools.removeTrailSlash : import_swiss_ak.fn.noact);
  };
  ...
  $$2.findModified = async (dir = ".", options = {}) => {
    ...
    return utils.intoLines(result).map(import_swiss_node.PathTools.removeDoubleSlashes).filter((str) => !str.includes(".Trash")).map((line) => {
  ...
  $$2.sync = (a, b, progressBarOpts) => $$2.rsync(import_swiss_node.PathTools.trailSlash(a), import_swiss_node.PathTools.trailSlash(b), ["--delete"], progressBarOpts);
  $$2.isFileExist = async (file) => await import_zx.$`[[ -f ${file} ]]`.exitCode === 0;
  $$2.isDirExist = async (dir) => await import_zx.$`[[ -d ${dir} ]]`.exitCode === 0;
  $$2.readFile = (filepath) => (0, import_swiss_ak.retryOr)("", 2, 100, true, () => fs.readFile(filepath, { encoding: "utf8" }));
  $$2.writeFile = (filepath, contents) => (0, import_swiss_ak.retryOr)(void 0, 2, 100, true, () => fs.writeFile(filepath, contents, { encoding: "utf8" }));
...
var ffmpegTools;
((ffmpegTools2) => {
  ffmpegTools2.ffmpeg = async (command = () => import_zx2.$`ffmpeg -progress pr.txt`, progressFileName = "pr.txt", totalFrames = 1, progressBarOpts = {}) => {
  ...
  ffmpegTools2.getProbeValue = async (file, propertyName) => (await import_zx2.$`ffprobe -select_streams v -show_streams ${file} 2>/dev/null | grep ${propertyName} | head -n 1 | sed -e 's/.*=//'`).toString();

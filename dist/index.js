...
var transpose = (rows) => {
  return import_swiss_ak5.ArrayUtils.zip(...rows);
};
...
var left = (item, width = table.utils.getTerminalWidth(), replaceChar = " ", forceWidth = true) => getLogLines(item).map((line) => pad(line, 0, forceWidth ? correctWidth(width) - (0, import_string_width.default)(line) : 0, replaceChar)).join(NEW_LINE);
var right = (item, width = table.utils.getTerminalWidth(), replaceChar = " ", forceWidth = true) => getLogLines(item).map((line) => pad(line, correctWidth(width) - (0, import_string_width.default)(line), 0, replaceChar)).join(NEW_LINE);
var alignFunc = {
  left,
  center,
  right
